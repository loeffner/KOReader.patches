...
local SHOW_TEMPERATURE = true  -- Show temperature text below icon
local SHOW_CONDITION_TEXT = true  -- Show condition text (e.g., "Sunny", "Cloudy")
...
local function patchWeather(weather_plugin)
    ...
    local function fetchWeatherData()
        ...
        local api_key = G_reader_settings:readSetting("weather_api_key") or "2eec368fb9a149dd8a4224549212507"
        ...
        logger.dbg("Weather lockscreen: Clock style:", clock_style)
        -- Initialize weather API
        local api = WeatherApi:new{ api_key = api_key }
        logger.dbg("Weather lockscreen: Clock style:", clock_style)        -- Fetch current weather + 2 day forecast (to get hourly data)
        ...
        if result and result.current and not result.error then
            ...
            local current_data = {
                ...
                condition = condition,
                time = result.location and result.location.localtime or ""
            }

            -- Get current hour from localtime (format: "2025-11-01 16:12")
            local current_hour = 0
            if result.location and result.location.localtime then
                local _, _, hour_str = result.location.localtime:find("(%d+):%d+$")
                current_hour = tonumber(hour_str) or 0
            end
            logger.dbg("Weather lockscreen: Current hour:", current_hour)
                        ...
                        if hour then
                            for _, target_hour in ipairs(target_hours) do
                                if hour == target_hour then
                                    ...
                                    local h_temp
                                    if temp_scale == "C" then
                                        h_temp = math.floor(hour_data.temp_c) .. "°"
                                    else
                                        h_temp = math.floor(hour_data.temp_f) .. "°"
                                    end

                                    -- Format hour based on clock style
                                    local hour_label
                                    if clock_style == "12" then
                                        if target_hour == 0 then
                                            hour_label = "12 AM"
                                        elseif target_hour < 12 then
                                            hour_label = target_hour .. " AM"
                                        elseif target_hour == 12 then
                                            hour_label = "12 PM"
                                        else
                                            hour_label = (target_hour - 12) .. " PM"
                                        end
                                    else
                                        hour_label = target_hour .. ":00"
                                    end
    ...
    local function createFallbackWidget()
        ...
        -- Check if the icon file exists, if not try relative to koreader directory
    ...
    local function createWeatherLockscreenWidget()
        ...
        if not weather_data or not weather_data.current or not weather_data.current.icon_path then
            logger.warn("Weather lockscreen: No weather data or icon path available, using fallback")
        ...
        if SHOW_TEMPERATURE and weather_data.current.temperature then
        ...
        if SHOW_CONDITION_TEXT and weather_data.current.condition then
            table.insert(current_widgets, TextWidget:new{
